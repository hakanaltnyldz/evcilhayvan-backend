...
export async function createMatchRequest(req, res) {
  try {
    ...
    // FCM push to target pet owner
    const targetOwnerId = String(advert.ownerId._id || advert.ownerId);
    const fcmSenderName = populated.fromUserId?.name || "Biri";
    sendPush([targetOwnerId], {
      title: "Yeni Eslestirme Istegi",
      body: `${fcmSenderName} hayvani icin eslestirme istegi gonderdi`,
      data: { type: "match_request", requestId: String(request._id) },
    }).catch(() => {});
...
export async function updateMatchRequestStatus(req, res) {
  try {
    ...
    // CRITICAL FIX: Handle both populated and non-populated user IDs
    ...
    if (io?.to) {
      // Use the extracted IDs from above (handles both populated and non-populated)
      const fromUserRoom = `user:${String(fromUserId)}`;
      const toUserRoom = `user:${String(toUserId)}`;
      ...
      if (action === "accept") {
        ...
        // Notify about new conversation (populate for frontend)
        ...
        io.to(toUserRoom).emit("conversation:created", {
          conversationId,
          conversation: populatedConversation
        });
        io.to(fromUserRoom).emit("conversation:created", {
          conversationId,
          conversation: populatedConversation
        });
    ...
    // FCM push for accept/reject
    const fcmResponderName = populated.toUserId?.name || "Kullanici";
    if (action === "accept") {
      sendPush([String(fromUserId)], {
        title: "Eslestirme Kabul Edildi!",
        body: `${fcmResponderName} eslestirme isteginizi kabul etti`,
        data: { type: "match_accepted", conversationId: conversationId || "" },
      }).catch(() => {});
    } else if (action === "reject") {
      sendPush([String(fromUserId)], {
        title: "Eslestirme Istegi",
        body: `${fcmResponderName} eslestirme isteginizi reddetti`,
        data: { type: "match_rejected", requestId: String(request._id) },
      }).catch(() => {});
    }
...
export async function getMatchingProfiles(req, res) {
  try {
    const userId = req.user.sub;
    const { species, gender, maxDistanceKm, minAgeMonths, maxAgeMonths, breed, vaccinated } = req.query;
    ...
    const myPets = await Pet.find({ ownerId: userId, isActive: true }).select("_id location species breed ageMonths isVaccinated");
    ...
    if (minAgeMonths) matchFilter.ageMonths = { ...matchFilter.ageMonths, $gte: Number(minAgeMonths) };
    if (maxAgeMonths) matchFilter.ageMonths = { ...matchFilter.ageMonths, $lte: Number(maxAgeMonths) };
    if (breed) matchFilter.breed = { $regex: breed, $options: "i" };
    if (vaccinated === "true") matchFilter.isVaccinated = true;
    ...
    const profiles = rawPets.map((pet) => {
      ...
      let score = 0;
      // Breed match (25 pts)
      if (myPrimaryPet?.breed && pet.breed &&
          String(myPrimaryPet.breed).toLowerCase() === String(pet.breed).toLowerCase()) {
        score += 25;
      }
      // Age proximity (max 20 pts)
      if (myPrimaryPet?.ageMonths != null && pet.ageMonths != null) {
        const ageDiff = Math.abs((myPrimaryPet.ageMonths || 0) - (pet.ageMonths || 0));
        if (ageDiff < 12) score += 20;
        else if (ageDiff < 24) score += 10;
      }
      // Vaccinated (10 pts)
      if (pet.isVaccinated) score += 10;
      // Has photos (5 pts)
      if ((pet.photos?.length || 0) > 0) score += 5;
      // Distance score (max 40 pts)
      if (distanceKm < 1) score += 40;
      else if (distanceKm < 5) score += 30;
      else if (distanceKm < 20) score += 20;
    ...
    profiles.sort((a, b) => b.score - a.score || a.distanceKm - b.distanceKm);
