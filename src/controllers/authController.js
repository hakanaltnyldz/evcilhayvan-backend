...
export async function register(req, res) {
  try {
    ...
    try {
      ...
      // Email gönderilemedi — kullanıcıyı direkt verified yap
      console.warn("[Register] Email gönderilemedi, direkt doğrulandı:", err.message);
    ...
    // Email gönderilemedi → direkt token döndür
    const tokens = await issueTokens(user);
    return sendOk(res, 201, {
...
export async function unregisterFcmToken(req, res) {
  try {
    ...
    await User.findByIdAndUpdate(req.user.sub, { $pull: { fcmTokens: token } });
    return sendOk(res, 200, { message: "FCM token silindi" });
  } catch (err) {
    return sendError(res, 500, "Token silinemedi", "internal_error", err.message);
  }
}
