...
class _EventDetailScreenState extends ConsumerState<EventDetailScreen> {
  ...
  Future<void> _loadMyStatus() async {
    try {
      final status = await ref
          ...
          .myAttendanceStatus(widget.eventId);
  ...
  Future<void> _attend(String status) async {
    ...
    try {
      await ref
          .read(eventRepositoryProvider)
          .attendEvent(widget.eventId, status);
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                      ...
                      _InfoCard(
                        children: [
                          if (event.venueName != null)
                            _InfoRow(icon: Icons.place, text: event.venueName!),
                          if (event.address != null)
                            _InfoRow(icon: Icons.map, text: event.address!),
                        ],
                      ),
                    ...
                    // Species
                    if (event.speciesAllowed.isNotEmpty &&
                        !event.speciesAllowed.contains('all')) ...[
                      Text(
                        'Katilabilecek Hayvanlar',
                        style: theme.textTheme.titleMedium?.copyWith(
                          fontWeight: FontWeight.bold,
                        ),
                      ),
  ...
  Widget _buildSliverAppBar(PetEventModel event, ThemeData theme) {
    final photos = event.photos.isNotEmpty
        ? event.photos
        : (event.coverPhoto != null ? [event.coverPhoto!] : <String>[]);
    return SliverAppBar(
              ...
              background: Stack(
                children: [
                  PageView.builder(
                    ...
                    itemBuilder: (_, i) {
                      final url = photos[i].startsWith('http')
                          ? photos[i]
                          : '$apiBaseUrl${photos[i]}';
...
class _OrganizerCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final avatarUrl = event.organizerAvatar != null
        ? (event.organizerAvatar!.startsWith('http')
              ? event.organizerAvatar!
              : '$apiBaseUrl${event.organizerAvatar}')
        : null;
    return Row(
      children: [
        CircleAvatar(
                    ...
                    children: [
                      Expanded(
                        ...
                        child: ElevatedButton.icon(
                          onPressed: (event.isFull && myStatus != 'going')
                              ? null
                              : () => onAttend('going'),
                          icon: Icon(
                            myStatus == 'going'
                          ...
                          label: Text(
                            myStatus == 'going' ? 'Katiliyorum' : 'Katil',
                          ),
                          style: ElevatedButton.styleFrom(
                            backgroundColor: myStatus == 'going'
                      ...
                      Expanded(
                        child: OutlinedButton.icon(
                          ...
                          style: OutlinedButton.styleFrom(
                            foregroundColor: myStatus == 'interested'
                                ? Colors.amber.shade700
                                : null,
                            side: myStatus == 'interested'
