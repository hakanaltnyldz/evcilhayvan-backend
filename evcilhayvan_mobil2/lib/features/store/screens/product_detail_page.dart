...
class _ProductDetailPageState extends ConsumerState<ProductDetailPage> {
  ...
  Future<void> _addToCart(ProductModel product) async {
    if (_isOwnProduct(product)) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Kendi urunlerinizi sepete ekleyemezsiniz'),
    ...
    if (product.stock <= 0) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Bu ürün stokta yok'),
    ...
    if (_quantity > product.stock) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Maksimum ${product.stock} adet ekleyebilirsiniz'),
          backgroundColor: Colors.orange,
        ),
      );
      return;
  ...
  void _shareProduct(ProductModel product) {
    // Share fonksiyonu için share_plus paketi gerekli
    // TODO: pubspec.yaml'a eklenecek: share_plus: ^7.2.1
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        ...
        child: SafeArea(
          child: detail.when(
            data: (product) {
              ...
              final displayDescription = description.isNotEmpty ? description : 'Açıklama yok';
                      ...
                      child: Container(
                        ...
                        child: const Row(
                          ...
                          children: [
                            ...
                            Text(
                              'Bu sizin ürününüz',
                              style: TextStyle(fontWeight: FontWeight.w700, color: Colors.orange, fontSize: 15),
                            ),
                          ],
                        ),
                      ),
            ...
            error: (e, _) => const Center(child: Text('Ürün bulunamadı.')),
          ),
        ),
      ),
    );
