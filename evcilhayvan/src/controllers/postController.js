...
export async function getFeed(req, res) {
  try {
    ...
    const posts = rawPosts.map((post) => serializePost(post, viewerId));
...
export async function addComment(req, res) {
  try {
    ...
    const savedComment = serializeComment(
      post.comments[post.comments.length - 1],
      post.comments[post.comments.length - 1].toJSON()
    );

    await notifyPostComment(post, user, savedComment);
...
export async function replyToComment(req, res) {
  try {
    ...
    const { text } = req.body;
    if (!text?.trim()) {
      return sendError(res, 400, "Yanit metni gerekli", "validation_error");
    }
    ...
    const savedReply = serializeReply(
      comment.replies[comment.replies.length - 1],
      comment.replies[comment.replies.length - 1].toJSON()
    );

    await notifyPostReply(post, comment, user, savedReply);
