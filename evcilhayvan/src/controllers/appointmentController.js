...
export async function createAppointment(req, res) {
  try {
    ...
    if (populated?.veterinaryId?.userId) {
      await emitAppointmentNotification({
        appointment: populated,
        eventName: "appointment:new",
        socketPayload: {
          appointmentId: String(populated._id),
          veterinaryName: populated.veterinaryId?.name || "",
          petName: populated.petId?.name || "",
          ownerName: populated.userId?.name || "",
          date: populated.date,
        },
        title: "Yeni Randevu Talebi",
        body: `${populated.petId?.name || "Bir pet"} icin yeni randevu talebi geldi.`,
        pushType: "appointment_new",
...
export async function updateAppointmentStatus(req, res) {
  try {
    ...
    if (viewerRole !== "owner" && typeof vetNotes === "string" && vetNotes.trim()) {
